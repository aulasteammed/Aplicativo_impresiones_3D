...
| **Mantenimiento** | A Fecha · B Impresora ID · C **Naturaleza** (`preventivo`/`correctivo`) · D Descripción · E **Costo (COP)** · F Responsable · G Programación (`ninguna`/`fecha`/`horas`) · H Próxima fecha · I Cada N horas · J Horas base (horas de la impresora al registrar) · K **Categoría de gasto** (`consumible`/`repuesto`/`servicio`/vacío) |
...
> **Tipos de mantenimiento (dos ejes independientes)**: la **Naturaleza** (C) responde a *por qué* se hizo — **preventivo** o **correctivo** — y la **Categoría de gasto** (K) a *en qué se gastó* — **consumible**, **repuesto**, **servicio** o vacío (sin compra). Son independientes: p. ej. comprar grasa para un preventivo = Preventivo + Consumible. El Dashboard desglosa el costo por categoría de gasto y por naturaleza.
>
